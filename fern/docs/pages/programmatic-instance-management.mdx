...
<Steps>
  ### Create an SSH key
    ...
    The script reads the public key located at `~/.ssh/id_rsa.pub`, then sends a request to the [`POST /ssh_keys`](/api-reference/ssh_keys/create) endpoint to create a SSH key named `"python_testkey"`. This SSH key is a copy of the public key that was read.
  ...
  ### Create an instance
    ...
    The script creates an instance by sending a request to the [`POST /instances`](/api-reference/ssh_keys/create) endpoint.
  ...
  ### Check the instance status
    ...
    Initially, the `status` of the instance is `pending`. The script repeatedly checks its `status` until it returns `running`, by sending requests to the [`GET /instances`](/api-reference/instances/list) endpoint and parsing the result. 
...
r = requests.post(
    api_url + "ssh_keys",
    headers=headers,
    json={"name": "python_testkey", "public_key": public_key}, # Create a SSH key with the name "python_testkey", using the provided public key
)
