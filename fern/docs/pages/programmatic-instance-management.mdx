...
<Steps>
  ### Create an SSH key
    ...
    The script reads the public key located at `~/.ssh/id_rsa.pub`, then sends a request to the [`POST /ssh_keys`](/api-reference/ssh_keys/create) endpoint to create a new SSH key named `"python_testkey"`. This SSH key is a copy of the public key that was read.
