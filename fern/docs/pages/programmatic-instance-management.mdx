...
<Steps>
  ### Create an SSH key
    ...
    The script reads the public key located at `~/.ssh/id_rsa.pub`, then sends a request to the [`POST /ssh_keys`](/api-reference/ssh-keys/create) endpoint to create a SSH key named `"python_testkey"`. This SSH key is a copy of the public key that was read.
  ...
  ### Create an instance
    ...
    The script creates an instance by sending a request to the [`POST /instances`](/api-reference/ssh-keys/create) endpoint.
  ...
  ### Check the instance status
    ...
    Initially, the instance's `status` is `pending`. The script repeatedly checks this `status` until it is `running` by sending requests to the [`GET /instances`](/api-reference/instances/list) endpoint and parsing the response. 
