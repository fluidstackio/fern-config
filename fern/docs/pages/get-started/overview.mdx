---
title: FluidStack Documentation
subtitle: Everything you need to know to use FluidStack's GPU instances
---

<Cards>
    <Card 
        title="Quickstart"
        icon="fa-regular fa-play" 
        href="/get-started/quickstart" 
    >
        Learn how to create, access, and delete your first instance
    </Card>
    <Card
        title="Programmatic Instance Management"
        icon="fa-regular fa-book" 
        href="/instances/programmatic-instance-management"
    >
        Examples of how to use the FluidStack API in Python
    </Card>
    <Card 
      title="Authentication" 
      icon="fa-solid fa-key" 
      href="/get-started/api-overview#authentication" 
    >
        How to authenticate with our API
    </Card>
    <Card 
        title="API Reference" 
        icon="fa-solid fa-code" 
        href="/api-reference" 
    >
        Learn about the FluidStack API endpoints
    </Card>
</Cards>