...
<Info>    The FluidStack API base URL is `https://platform.fluidstack.io`</Info>
    
    

## Authentication

Whenever you make a call to the FluidStack API, include your API key in the request headers. If you are using any of our SDKs, you can provider the API key as a parameter. 

The example below shows how to call the [`GET /instances`](/api-reference/instances/list) endpoint, which returns the list of instances on your account. The second line demonstrates how to include your API key in the headers:
...
Initially, the instance `status` will be `pending`. The `status` changes to `running` and an IP address is assigned once the instance is created and available.
...
Once the instance is running, you can connect to it using SSH. Use the instance's IP address and the private key that corresponds to the public key you used when creating the SSH key for this instance.
