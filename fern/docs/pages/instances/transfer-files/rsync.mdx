...
By default, `rsync` overwrites existing files at the destination without prompting for confirmation. To prevent this behavior, use the `--ignore-existing` option:
