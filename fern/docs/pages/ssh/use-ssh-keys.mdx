...
You must provide an [SSH key](/ssh/what-is-ssh#ssh-keys) when you create a cloud-based GPU instance with FluidStack. Once the instance is running, that SSH key is used to authenticate for remote command-line access to the instance.
...
```bash title="Example request"
  curl -X DELETE https://platform.fluidstack.io/ssh_keys/my_ssh_key \
...
Use your private key to authenticate when connecting to a running instance via SSH. The instance has an SSH key associated with it, provided at the time that the instance was created. The private key must correspond to that SSH key.
